...
WITH RECURSIVE recommenders(recommender, member, level) AS (
    SELECT recommendedby, memid, 0
    FROM cd.members
    WHERE memid = 12 OR memid = 22

    UNION ALL

    SELECT mems.recommendedby, recs.member, recs.level + 1
